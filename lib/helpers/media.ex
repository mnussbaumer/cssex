defmodule CSSEx.Helpers.Media do
  ...
  def do_parse(<<41, rem::binary>>, data, %{acc: acc, parenthesis: p, p_acc: p_acc} = state) when p > 0 do
	...
	{nil, new_p_acc_2} -> %{new_state | p_acc: new_p_acc_2, acc: [acc | ["(", processed, ")"]]}

	{previous_acc, new_p_acc_2} ->
	  %{new_state | p_acc: Map.put(new_p_acc_2, previous_p, [previous_acc | ["(", processed, ")"]])}
      end
  ...
  def do_parse(<<char::binary-size(1), rem::binary>>, data, %{parenthesis: p, p_acc: p_acc} = state) when p > 0 do
